...
A missing scope does not fail at sign-in. It surfaces later as a 403 partway through a sync or a provisioning action, so grant the full set. Check `tableau:permissions:read` twice: grant sync reads project, workbook, and view ACLs on every run, and `tableau:content:read` does not cover those reads.
</Step>
<Step>
