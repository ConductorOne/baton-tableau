...
- **Projects with `LockedToProject`**: When a project's content permissions are set to `LockedToProject`, workbooks inside that project inherit the project's permissions. The connector syncs these inherited permissions as grants on the individual workbooks, with group membership fully expanded so each user appears as a principal.
